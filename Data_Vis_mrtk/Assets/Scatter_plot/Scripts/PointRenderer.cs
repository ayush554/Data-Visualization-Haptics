...
public class PointRenderer : MonoBehaviour {
    ...
    //Object which will contain instantiated clusters
    public GameObject ClusterHolder;

    //Object which will display the currently selected cluster
    public GameObject clusterTracker;
	...
	private void PlacePrefabPoints()
	{
                ...
                for (var i = 0; i < pointList.Count; i++)
        {
            ...
            if (pointList[i]["cluster_center"].ToString() == "false")
            {
                if (pointList[i]["is_noise"].ToString() == "true")
                {
                    //Debug.Log("fsdad");
                    dataPoint = Instantiate(HapticPrefab, Vector3.zero, Quaternion.identity);
                }
                else
                {
            ...
            {
                dataPoint = Instantiate(ClusterPrefab, Vector3.zero, Quaternion.identity);
            }
            ...
            {
                dataPoint.transform.parent = ClusterHolder.transform;
            }
            ...
            if (pointList[i]["cluster_center"].ToString() == "false")
            {
                if (pointList[i]["is_noise"].ToString() == "true")
                {
                    dataPoint.transform.localScale = new Vector3(noiseScale, noiseScale, noiseScale);
                }
                else
                {
            ...
            {
                //Set the scale to what the cluster scale should be at
                dataPoint.transform.localScale = new Vector3(2.5f, 2.5f, 2.5f);
            }
            string dataPointName = "";
            // Converts index to string to name the point the index number
            if (pointList[i]["cluster_center"].ToString() == "false")
            {
                dataPointName = i.ToString();
            } else
            {
                dataPointName = pointList[i]["cluster_center"].ToString();
            }

            // Assigns name to the prefab
            dataPoint.transform.name = dataPointName;

            if (renderPrefabsWithColor == true && pointList[i]["cluster_center"].ToString() == "false")
            {
                // Sets color according to x/y/z value
                dataPoint.GetComponent<Renderer>().material.color = new Color32(150,150,150,255);
                ...
                dataPoint.GetComponent<Renderer>().material.SetColor("_EmissionColor", new Color32(150, 150, 150, 255));

            }
            
            //Make sure the event for the cluster center sends the correct string
            
            if(pointList[i]["cluster_center"].ToString() != "false")
            {
                if (clusterTracker != null) {
                    var hapticRegion = dataPoint.GetComponent<HapticTriggerRegion>();
                    UnityEngine.Events.UnityAction clusterTrackAddFunc = () => { clusterTracker.GetComponent<ClusterTracker>().AddCurrentCluster(dataPointName); };
                    hapticRegion.OnEnter += clusterTrackAddFunc;
                    UnityEngine.Events.UnityAction clusterTrackRemoveFunc = () => { clusterTracker.GetComponent<ClusterTracker>().RemoveCurrentCluster(dataPointName); };
                    hapticRegion.OnLeave += clusterTrackRemoveFunc;
                }
            }
    ...
    private void CreateParticles()
    {
       ...
       // Debug.Log("Row Count is " + rowCount);
        ...
        for (int i = 0; i < pointList.Count; i++)
        {
            ...
            //GlowColor = 
            // Set point color
            //particlePoints[i].startColor = new Color32(150, 150, 150, 255);
